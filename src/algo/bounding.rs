...
fn circle_contains_all_points(circle: &Circle, points: &[Point]) -> bool {
    const EPS: f64 = 1e-10;
    points.iter().all(|&p| (p - circle.c).norm() <= circle.r + EPS)
}
...
fn _point_in_circle_robust(p: Point, circle_points: &[Point]) -> bool {
    if circle_points.len() != 3 {
        ...
        let center = circle_points.iter().fold(point(0.0, 0.0), |acc, &pt| acc + pt) / circle_points.len() as f64;
        let radius = circle_points.iter().map(|&pt| (pt - center).norm()).fold(0.0, f64::max);
        return (p - center).norm() <= radius + 1e-10;
    }
    
    // Use incircle predicate for robust inside/outside test
    let result = incircle(
        Coord { x: circle_points[0].x, y: circle_points[0].y },
        Coord { x: circle_points[1].x, y: circle_points[1].y },
        Coord { x: circle_points[2].x, y: circle_points[2].y },
        Coord { x: p.x, y: p.y },
    );
