...
pub fn poly1000_circular() -> Arcline {
    ...
    for _ in 0..num_arcs {
        ...
        let arc = arc_from_bulge(Point::new(start_x, start_y), Point::new(end_x, end_y), -0.2);
...
pub fn poly1000_grid() -> Arcline {
    ...
    for _ in 0..num_arcs {
        ...
        let arc = arc_from_bulge(Point::new(start_x, start_y), Point::new(end_x, end_y), 0.0);
...
mod tests {
    ...
    fn test_poly1000_len() {
        ...
        assert!(
            poly.len() >= 980 && poly.len() <= 985,
            "Expected ~982 arcs, got {}",
            poly.len()
        );
    }
    ...
    fn test_poly1000_closed() {
        ...
        if !poly.is_empty() {
            ...
            let dist = ((start_point.x - end_point.x).powi(2)
    ...
    fn test_poly1000_no_self_intersections() {
        ...
        if has_intersection {
            ...
            println!(
                "poly1000 has {} self-intersection points",
                intersections.len()
            );
